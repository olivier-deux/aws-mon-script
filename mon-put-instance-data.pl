...
if ($report_disk_space)
{
  ...
  foreach my $line (@df)
  {
    ...
    if ($report_disk_util) {
      ...
      add_metric('DiskSpaceUtilization', 'Percent', $disk_util, $mount);
    }
    if ($report_disk_used) {
      add_metric('DiskSpaceUsed', $disk_units, $disk_used / $disk_unit_div, $mount);
    }
    if ($report_disk_avail) {
      add_metric('DiskSpaceAvailable', $disk_units, $disk_avail / $disk_unit_div, $mount);
    }
  }
}
